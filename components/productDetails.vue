<template>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24">
          ...
          <div class="flex flex-col animate-slide-in-right">
            ...
            <PaymentTypeSelector
              ...
              :shipping-fee="25" 
              :total-price="totalPrice + 25"
              v-model="paymentType"
              @installment-selected="handleInstallmentSelected"
              @full-payment-selected="handleFullPaymentSelected"
            />
              ...
              <div class="flex flex-col sm:flex-row gap-4">
                ...
                >
                  ...
                  <span class="font-medium">
                    {{ !selectedSize ? 'Select Size First' : product.isAvailable ? getAddToCartText() : 'Unavailable' }}
                  </span>
                </button>
                <button 
              ...
              <div class="border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
                ...
                >
                  <div v-if="openAccordions.shippingInfo" class="px-6 pb-6 border-t border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                    <div class="pt-4 prose prose-sm max-w-none text-gray-700">
                      <div v-html="product.shippingInfo"></div>
                      <div class="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div class="flex items-center text-blue-700">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2">
                            <path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/>
                          </svg>
                          <span class="font-semibold">Fixed Shipping Fee: $25.00</span>
                        </div>
                        <p class="text-sm text-blue-600 mt-1">This fee is included in all payment plans including installments.</p>
                      </div>
                    </div>
                  </div>
                </transition>
  ...
  // Payment handlers
  const handleInstallmentSelected = (data: { plan: InstallmentPlan; calculation: InstallmentCalculation }) => {
    selectedInstallmentPlan.value = data.plan
    installmentCalculation.value = data.calculation
    
    showToast({
      title: "Installment Plan Selected",
      message: `${data.plan.name} selected with ${data.plan.downPaymentPercentage}% down payment`,
      toastType: "success",
      duration: 3000
    });
  }
  ...
  const handleFullPaymentSelected = () => {
    ...
    installmentCalculation.value = null
    
    showToast({
      title: "Full Payment Selected",
      message: "You will pay the full amount at checkout",
      toastType: "info",
      duration: 3000
    });
  }
  ...
  const addToCart = () => {
    if (product.value && product.value.isAvailable && selectedSize.value) {
      ...
      const toastMessage = paymentType.value === 'installment' && selectedInstallmentPlan.value
        ? `${quantity.value} × ${product.value.name} (${selectedSize.value.size}) added with ${selectedInstallmentPlan.value.name}`
        : `${quantity.value} × ${product.value.name} (${selectedSize.value.size}) added to cart`
  ...
  const addRelatedToCart = (relatedProduct: any) => {
    ...
    addItemToCart({
      ...
      paymentType: 'full' // Default to full payment for related products
    })
  ...
  .animation-delay-650 { animation-delay: 0.65s; opacity: 0; }
